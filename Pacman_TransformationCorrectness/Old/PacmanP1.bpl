...
&&	grid2 != null && read($srcHeap,grid2,alloc) && dtype(grid2) == pacman$Grid
	==>	
	(forall gem,pac:ref ::
		gem != null && read($srcHeap,gem,alloc) && dtype(gem) == pacman$Gem
		&&	pac != null && read($srcHeap,pac,alloc) && dtype(pac) == pacman$Pacman
		&&  read($srcHeap,grid1,pacman$Grid.hasGem) == gem
		&&  read($srcHeap,grid2,pacman$Grid.hasPlayer) == pac
		==>
		reachable($srcHeap, grid1, grid2)
	)	
);
